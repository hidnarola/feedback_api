    ...
    getPostDetails: function (json, callback) {
        ...
        async.parallel({
            ...
            positive_location_votes: function (callback) {
                con.connection.query("select COALESCE(SUM(CASE WHEN vote_value = 1 THEN 1 ELSE 0 END),0) positive_votes,city,state,country FROM feed_votes where feed_id= ? AND vote_value = 1 group by city,state,country", [json.feed_id], function (err, result_votes) {
                    if (err) {
                        console.log(err);
                    } else {
                        callback(null, result_votes)
            ...
            negative_location_votes: function (callback) {
                con.connection.query("select COALESCE(SUM(CASE WHEN vote_value = 0 THEN 1 ELSE 0 END),0) positive_votes,city,state,country FROM feed_votes where feed_id= ? AND vote_value = 0 group by city,state,country", [json.feed_id], function (err, result_votes) {
                    if (err) {
                        console.log(err);
                    } else {
                        callback(null, result_votes)
