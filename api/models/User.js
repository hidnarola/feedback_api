...
var User = {
    ...
    getAllFeeds: function (json, callback) {
        ...
        var sql = "SELECT f.id,f.feed_text,DATE_FORMAT(f.created,'%Y-%m-%e %H:%i:%s') as feed_created,f.flag_type,fv.id as feed_vote_id,fv.user_id, fv.vote_value ,fv.city,fv.state,fv.country,DATE_FORMAT(fv.created,'%Y-%m-%e %H:%i:%s') as created,fp.id as flag_user"
        ...
        sql += " LEFT JOIN flag_post fp ON fp.feed_id = f.id AND fp.user_id= " + json.device_user_id;
        sql += " WHERE ";
            ...
            if (err) {
                ...
                var result_arr = _.map(result_feeds, function (obj) {
                    ...
                    var json = {
                        ...
                        "flag_user": (obj.flag_user > 0) ? true : false,
                    };
                    json['user_vote'] = {}
                    if (obj.feed_vote_id) {
